...
public class GetStockPricesUsingThreads {
	...
	public static class Builder implements Runnable {
		...
		private void isIBMStock() {
			if (companyNameOfStock == "IBM")
				((StockGrabber) stockGrabber).setIBMStockPrice(stockPrice);
		}

		private void isAppleStock() {
			if (companyNameOfStock == "APPLE")
				((StockGrabber) stockGrabber).setAppleStockPrice(stockPrice);
			;
		}

		private void isGoogleStock() {
			if (companyNameOfStock == "GOOGLE")
				((StockGrabber) stockGrabber).setGoogleStockPrice(stockPrice);
			;
		}

		private void printStockPrices(double randomStockPriceMarketChange) {
			System.out.println(companyNameOfStock + " Stock: "
					+ formatPriceToTwoDecimalPlaces().format((stockPrice + randomStockPriceMarketChange)) + "\nChange: "
					+ formatPriceToTwoDecimalPlaces().format(randomStockPriceMarketChange));
		}
