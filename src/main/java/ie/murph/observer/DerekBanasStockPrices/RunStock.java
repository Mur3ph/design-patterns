...
public class RunStock {
	...
	private static Runnable getIBMStockPrices(StockGrabber stockGrabber) {
		return new GetStockPricesUsingThreads.Builder().StockGrabber(stockGrabber).ThreadStartTime(2)
				.CompanyNameOfStock("IBM").StockPrice(197.00);
		// return new GetStockPricesUsingThreads(stockGrabber, 2, "IBM",
		// 197.00);
	}
	...
	private static Runnable getAPPLEStockPrices(StockGrabber stockGrabber) {
		return new GetStockPricesUsingThreads.Builder().StockGrabber(stockGrabber).ThreadStartTime(4)
				.CompanyNameOfStock("APPLE").StockPrice(677.60);
		// return new GetStockPricesUsingThreads(stockGrabber, 4, "APPLE",
		// 677.60);
	}
	...
	private static Runnable getGOOGLEStockPrices(StockGrabber stockGrabber) {
		return new GetStockPricesUsingThreads.Builder().StockGrabber(stockGrabber).ThreadStartTime(6)
				.CompanyNameOfStock("GOOGLE").StockPrice(676.40);
		// return new GetStockPricesUsingThreads(stockGrabber, 6, "GOOGLE",
		// 676.40);
	}
}
